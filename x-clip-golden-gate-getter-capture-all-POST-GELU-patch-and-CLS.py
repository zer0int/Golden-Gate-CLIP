...
def get_clip_dimensions(clipmodel):
    ...
    num_features = model.visual.transformer.resblocks[-1].mlp.c_proj.in_features
...
class ClipNeuronCaptureHook:
    ...
    def hook_fn(self, module, input, output):
        # Aggregate activations across all tokens
        self.activations = output.view(output.size(0), -1).detach()  # Flatten activations
...
def register_hooks(model, layers_to_capture):
    ...
    for layer_idx in layers_to_capture:
        module = model.clip.visual.transformer.resblocks[layer_idx].mlp.c_proj
        #print(module)
...
def store_top_activations_to_csv(activations_list, filename="top_postGELU-patch_activations.csv"):
    ...
    for img_filename, img_activations in activations_list:
        ...
        for idx, values, indices in img_activations:
            activations_per_layer[idx].append(indices.tolist())
...
def store_common_features(common_features, filename="top_postGELU-CLS_activations-common.txt"):
...
print("\nTop activations for CLS stored successfully in top_postGELU-CLS_activations.csv")
print("Common features stored successfully in top_postGELU-CLS_activations-common.txt")
